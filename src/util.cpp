...
const char * const BITCOIN_CONF_FILENAME = "g0coin.conf";
const char * const BITCOIN_PID_FILENAME = "g0coind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "g0coin" is a composite category enabling all G0Coin-related debug output
            if(ptrCategory->count(string("g0coin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\G0CoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\G0CoinCore
    // Mac: ~/Library/Application Support/G0CoinCore
    // Unix: ~/.g0coincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "G0CoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/G0CoinCore";
#else
    // Unix
    return pathRet / ".g0coincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty g0coin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override g0coin.conf
