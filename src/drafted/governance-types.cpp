...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "G0CoinNetwork") type = G0CoinNetwork;
    if(strType == "G0CoinNetworkVariable") type = G0CoinNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == G0CoinNetwork) s = "G0CoinNetwork";
    if(type == G0CoinNetworkVariable) s = "G0CoinNetworkVariable";
