...
*Setup instructions for a Gitian build of G0Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the G0Coin
...
- [Building G0Coin Core](#building-g0coin-core)
...
Clone the git repositories for G0Coin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/G0CoinTeam/G0Coin
```
...
in the G0Coin Core repository under 'Fetch and build inputs' to install sources which require
...
To build G0Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the G0Coin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/g0coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/G0CoinTeam/G0Coin
...
For example:
```bash
URL=https://github.com/crowning-/g0coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit g0coin=${COMMIT} --url g0coin=${URL} ../g0coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit g0coin=${COMMIT} --url g0coin=${URL} ../g0coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit g0coin=${COMMIT} --url g0coin=${URL} ../g0coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the g0coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../g0coin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/g0coin.git
SIGPATH=/some/root/path/g0coin-detached-sigs.git

./bin/gbuild --url g0coin=${BTCPATH},signature=${SIGPATH} ../g0coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/g0coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/g0coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/g0coin-osx-build.assert
```
...
[g0coin/gitian.sigs](https://github.com/G0CoinTeam/gitian.sigs/) repository, or if that's not possible to create a pull
request.
